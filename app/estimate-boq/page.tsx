...
export default function Estimator() {
  ...
  // ARRAY STATES FOR MULTI-FLOOR LOOPING
  ...
  const handleSetupComplete = () => {
    ...
    // Initialize Layout & Openings Arrays for all floors
    const initialFloors = [];
  ...
  const handleStep4Continue = () => {
    ...
    if (activeFloor < totalFloorsCount - 1) {
        if (activeFloor === 0 && commercialGroundFloor && buildingType === 'apartment') {
            // Cannot copy Commercial GF to Residential Upper Floors. Auto-skip copy prompt.
            setActiveFloor(1);
            setCurrentStep(3);
            window.scrollTo(0,0);
        } else {
        ...
        setCurrentStep(5); // Proceed to Materials Pricing
        window.scrollTo(0,0);
    }
  };
          ...
          {!isAskingCopy ? (
                    ...
                    <div className="p-6 border border-gray-100 bg-gray-50/50 rounded-2xl">
                      ...
                      {buildingType === 'apartment' && (
                        <div className="mt-6 p-5 bg-blue-50 border border-blue-100 rounded-2xl">
                          ...
                          {(!commercialGroundFloor || totalFloorsCount > 1) && (
                                ...
                                <div className="space-y-4 mb-6">
                                  ...
                                  {flat.bedrooms.map((bed: any, bIdx: number) => (
                                      <div key={bed.id} className="flex flex-col md:flex-row gap-4 items-center p-4 border border-gray-100 bg-white rounded-xl">
                                        <span className="text-sm font-bold text-gray-500 md:w-24">Bed {bIdx + 1}</span>
                                  ...
                                  {flat.bathrooms.map((bath: any, bIdx: number) => (
                                      <div key={bath.id} className="p-4 border border-gray-100 bg-white rounded-xl space-y-4">
                                        <div className="flex flex-col md:flex-row gap-4 items-center">
                                            <span className="text-sm font-bold text-gray-500 md:w-24">Bath {bIdx + 1}</span>
                                            <div className="flex gap-4 w-full">
              ...
              {currentStep === 4 && (
                <div className="space-y-6 animate-in fade-in slide-in-from-bottom-4 duration-500 max-w-4xl mx-auto">
                  ...
                  <div className="flex gap-4 pt-6">
                    ...
                    <button onClick={handleStep4Continue} className="w-2/3 bg-gray-900 text-white font-bold text-lg p-4 rounded-xl hover:bg-[#22c55e] transition-colors shadow-md">
                      Next Step ➔
                    </button>
                  </div>
                </div>
            ...
            <div className="space-y-8 py-16 text-center animate-in fade-in duration-500 max-w-lg mx-auto">
              ...
              <h2 className="text-2xl md:text-3xl font-bold text-gray-900 mb-2">
                Is the {activeFloor + 1 === 1 ? "1st Floor" : activeFloor + 1 === 2 ? "2nd Floor" : activeFloor + 1 === 3 ? "3rd Floor" : `${activeFloor + 1}th Floor`} layout the same as {floorsData[activeFloor]?.floorName}?
              </h2>
              <p className="text-gray-500 font-medium mb-4">Save time by duplicating the entire layout and measurements to all remaining floors.</p>
