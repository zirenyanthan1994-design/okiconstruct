...
export default function ProfilePage() {
  ...
  const handleUpdateProfile = async (e: React.FormEvent) => {
    ...
    try {
      ...
      setUserData({ ...(userData as any), name: editForm.name, phone: editForm.phone, address: editForm.address, avatar: editForm.avatar });
  ...
  const handleDeactivateAccount = async () => {
    ...
    if (userData.tier === 'premium' || userData.planStatus === 'premium') {
      ...
      try {
        await updateDoc(doc(db, "users", user.uid), { deactivationRequested: true, deactivationRequestDate: serverTimestamp() });
        alert("Deactivation request submitted. Our support team will process it and contact you shortly.");
      } catch (error: any) { alert("Error submitting request: " + error.message); }
      return;
    }
    ...
    try {
      ...
      if (error.code === 'auth/requires-recent-login') alert("For security reasons, please log out and log back in before deactivating your account.");
      else alert("Error deactivating account: " + error.message);
    }
  };
  ...
  const fetchSavedBOQs = async () => {
    ...
    } catch (error) { console.error("Error fetching BOQs:", error); } 
  ...
  const fetchSavedLayouts = async () => {
    ...
    try {
      ...
      const querySnapshot = await getDocs(q);
      const fetchedLayouts = querySnapshot.docs.map(doc => ({ id: doc.id, ...doc.data() }));
      fetchedLayouts.sort((a: any, b: any) => (b.createdAt?.seconds || 0) - (a.createdAt?.seconds || 0));
  ...
  return (
    <div className="min-h-screen bg-gray-50 flex flex-col font-sans">
      ...
      <main className="max-w-[1200px] mx-auto p-4 md:p-6 mt-8 flex-grow w-full">
        
        {/* --- HEADER & PROFILE CARD --- */}
        <div className="bg-white border border-gray-100 rounded-3xl p-8 shadow-sm relative mb-8 print:hidden">
          <div className="absolute top-6 right-6 flex items-center gap-4">
            <button onClick={() => setIsMessageCenterOpen(true)} className="relative p-2 text-gray-400 hover:text-[#22c55e] transition-colors bg-gray-50 hover:bg-green-50 rounded-full shadow-sm">
              <svg className="w-6 h-6" fill="none" stroke="currentColor" viewBox="0 0 24 24"><path strokeLinecap="round" strokeLinejoin="round" strokeWidth="2" d="M8 10h.01M12 10h.01M16 10h.01M9 16H5a2 2 0 01-2-2V6a2 2 0 012-2h14a2 2 0 012 2v8a2 2 0 01-2 2h-5l-5 5v-5z"></path></svg>
              <span className="absolute top-1 right-1 w-2.5 h-2.5 bg-red-500 border-2 border-white rounded-full"></span>
            </button>
            <button onClick={() => setIsEditModalOpen(true)} className="text-sm font-semibold text-gray-600 hover:text-gray-900 px-4 py-2 border border-gray-200 rounded-xl hover:bg-gray-50 transition-colors shadow-sm">Edit Profile</button>
          </div>
          ...
          <div className="flex flex-col md:flex-row items-center md:items-start gap-6 mt-8 md:mt-0">
            ...
            <div className="text-center md:text-left mt-2">
              ...
              <div className="flex flex-wrap items-center justify-center md:justify-start gap-2 mt-4">
                ...
                {paymentStatus && <span className="bg-amber-100 text-amber-800 px-3 py-1 rounded-lg text-xs font-bold uppercase tracking-widest border border-amber-200 animate-pulse">{paymentStatus}</span>}
                <span className="bg-gray-100 text-gray-700 px-3 py-1 rounded-lg text-xs font-semibold flex items-center gap-1">📞 {userData.phone}</span>
                <span className="bg-gray-100 text-gray-700 px-3 py-1 rounded-lg text-xs font-semibold flex items-center gap-1">📍 {userData.address || "No Address Provided"}</span>
              </div>
            </div>
          </div>
        </div>
        ...
        <div className="flex items-center justify-between mb-4 px-2 print:hidden">
          ...
          {activeTab !== 'overview' && !selectedBOQ && (
            <button onClick={() => setActiveTab('overview')} className="text-sm font-bold text-gray-500 hover:text-gray-900 flex items-center gap-2 transition-colors">⬅ Back to Overview</button>
          )}
        </div>
        
        {/* --- TAB: OVERVIEW (3 CARDS) --- */}
        {activeTab === 'overview' && (
          <div className="grid grid-cols-1 md:grid-cols-3 gap-6 mb-8 animate-in fade-in duration-300">
            ...
            <div className="bg-gray-900 border border-gray-800 rounded-3xl p-8 shadow-xl hover:shadow-2xl hover:-translate-y-1 transition-all group relative overflow-hidden flex flex-col">
              ...
              <div className="flex items-center justify-between mb-6 relative z-10">
                <div className="w-14 h-14 bg-[#22c55e]/20 text-[#22c55e] rounded-xl flex items-center justify-center"><span className="text-2xl">🏗️</span></div>
              ...
              <div className="flex items-end gap-3 relative z-10"><span className="text-4xl font-black text-white">Active</span></div>
            ...
            <div className="bg-gray-900 border border-gray-800 rounded-3xl p-8 shadow-xl hover:shadow-2xl hover:-translate-y-1 transition-all group relative overflow-hidden flex flex-col">
              ...
              <div className="flex items-center justify-between mb-6 relative z-10">
                <div className="w-14 h-14 bg-blue-500/20 text-blue-400 rounded-xl flex items-center justify-center"><span className="text-2xl">📊</span></div>
            ...
            <div className="bg-gray-900 border border-gray-800 rounded-3xl p-8 shadow-xl hover:shadow-2xl hover:-translate-y-1 transition-all group relative overflow-hidden flex flex-col">
              ...
              <div className="flex items-center justify-between mb-6 relative z-10">
                <div className="w-14 h-14 bg-purple-500/20 text-purple-400 rounded-xl flex items-center justify-center"><span className="text-2xl">📐</span></div>
                <button onClick={fetchSavedLayouts} className="text-sm font-bold text-purple-400 hover:text-white transition-colors bg-purple-500/10 hover:bg-purple-500 px-4 py-2 rounded-lg shadow-sm">View All ➔</button>
              </div>
              <h3 className="text-gray-400 font-bold text-sm uppercase tracking-wider mb-1 relative z-10">2D Layouts</h3>
              <div className="flex items-end gap-3 relative z-10"><span className="text-4xl font-black text-white">Active</span></div>
              <div className="mt-4 pt-4 border-t border-gray-800 text-sm font-medium text-gray-500 relative z-10">View and export your saved architectural blueprints.</div>
            </div>
          ...
          <>
            {!selectedBOQ ? (
              <div className="bg-white border border-gray-100 rounded-3xl p-6 shadow-sm animate-in fade-in duration-300">
                <h3 className="text-lg font-bold text-gray-900 mb-6 flex items-center gap-2"><span className="text-2xl">🏗️</span> Your Saved BOQ Estimates</h3>
                ...
                {isLoadingData ? (
                  ...
                  <div className="grid grid-cols-1 md:grid-cols-2 lg:grid-cols-3 gap-4">
                    {savedBOQs.map((boq) => (
                      <div key={boq.id} onClick={() => setSelectedBOQ(boq)} className="border border-gray-200 rounded-2xl p-5 hover:border-[#22c55e] transition-all group cursor-pointer shadow-sm hover:shadow-md hover:-translate-y-1 bg-white">
                        <div className="flex justify-between items-start mb-4">
                          <div>
                            <h4 className="font-bold text-gray-900 text-lg group-hover:text-[#22c55e] transition-colors">{boq.projectName}</h4>
                            <p className="text-xs font-semibold text-gray-400 uppercase tracking-widest mt-1">{new Date(boq.createdAt?.seconds * 1000).toLocaleDateString()}</p>
                          </div>
                          <span className="bg-blue-50 text-blue-600 px-3 py-1 rounded-lg text-xs font-bold">G+{boq.totalFloors - 1} Structure</span>
                        </div>
              ...
              <div className="animate-in fade-in duration-300 printable-boq">
                <button onClick={() => setSelectedBOQ(null)} className="mb-6 text-sm font-bold text-gray-500 hover:text-gray-900 flex items-center gap-2 transition-colors print:hidden bg-white px-4 py-2 rounded-lg border border-gray-200 shadow-sm">
                ...
                <div className="bg-white border border-gray-100 rounded-3xl p-6 md:p-10 shadow-lg print:shadow-none print:border-none print:p-0">
                  <div className="flex flex-col md:flex-row justify-between items-start md:items-center mb-8 border-b border-gray-100 pb-6 print:border-b-2">
                    <div>
                      ...
                      <div className="flex items-center gap-3 mt-2">
                        <span className="text-sm font-bold text-gray-500 uppercase tracking-widest">OkiConstruct Master BOQ</span>
                        <span className="text-gray-300">•</span>
                        <span className="text-sm font-medium text-gray-500">{new Date(selectedBOQ.createdAt?.seconds * 1000).toLocaleDateString()}</span>
                      </div>
                    </div>
                    
                    <div className="mt-4 md:mt-0 flex gap-3 print:hidden w-full md:w-auto">
                      <button onClick={() => window.location.href = `/estimate-boq?edit=${selectedBOQ.id}`} className="flex-1 md:flex-none bg-blue-50 text-blue-600 px-6 py-2.5 rounded-xl font-bold hover:bg-blue-100 transition-colors border border-blue-200">
                        ✏️ Edit Project
                      </button>
                      <button onClick={() => window.print()} className="flex-1 md:flex-none bg-gray-900 text-white px-6 py-2.5 rounded-xl font-semibold hover:bg-[#22c55e] transition-colors shadow-md flex items-center justify-center gap-2">
                  ...
                  <div className="bg-gray-900 text-white p-8 rounded-2xl text-center shadow-lg mt-12 print:shadow-none print:bg-white print:text-black print:border-2 print:border-black print:break-inside-avoid">
                    ...
                    <h2 className="text-5xl md:text-6xl font-black text-[#22c55e] print:text-black">₹ {Math.ceil(selectedBOQ.boqData?.grandTotal || 0).toLocaleString()}</h2>
                  </div>
                </div>
              </div>
            )}
        ...
        {activeTab === 'layouts' && (
          <div className="bg-white border border-gray-100 rounded-3xl p-6 shadow-sm animate-in fade-in duration-300">
            <h3 className="text-lg font-bold text-gray-900 mb-6 flex items-center gap-2"><span className="text-2xl">📐</span> Your Saved Layouts</h3>
            
            {isLoadingData ? (
              <p className="text-center text-gray-500 py-10 font-medium">Fetching your layouts from the cloud...</p>
            ) : savedLayouts.length === 0 ? (
              <div className="text-center py-10">
                <p className="text-gray-500 font-medium mb-4">You haven't saved any 2D Layouts yet.</p>
                <a href="/generate-2d-layout" className="inline-block bg-gray-900 text-white font-semibold px-6 py-2 rounded-xl hover:bg-[#22c55e] transition-colors shadow-md">Create New Layout</a>
              </div>
            ) : (
              <div className="grid grid-cols-1 md:grid-cols-2 lg:grid-cols-3 gap-4">
                {savedLayouts.map((layout) => (
                  <div key={layout.id} className="border border-gray-200 rounded-2xl p-5 hover:border-purple-500 transition-all group shadow-sm bg-white">
                    <div className="flex justify-between items-start mb-4">
                      <div>
                        <h4 className="font-bold text-gray-900 text-lg">{layout.projectName}</h4>
                        <p className="text-xs font-semibold text-gray-400 uppercase tracking-widest mt-1">{new Date(layout.createdAt?.seconds * 1000).toLocaleDateString()}</p>
                      </div>
                    ...
                    <div className="border-t border-gray-100 pt-4 mt-4 flex gap-2">
                      ...
                      <button onClick={() => {
                        ...
                        className="flex-1 bg-gray-900 text-white text-xs font-bold py-2 rounded-lg hover:bg-[#22c55e] transition-colors text-center shadow-sm"
                      >
                        Estimate BOQ
                      </button>
                    </div>
        ...
        {activeTab === 'ledgers' && (
          <div className="bg-white border border-gray-100 rounded-3xl p-6 shadow-sm animate-in fade-in duration-300">
             <h3 className="text-lg font-bold text-gray-900 mb-6 flex items-center gap-2"><span className="text-2xl">📊</span> Your Expense Ledgers</h3>
          ...
          <div className="bg-white w-full max-w-md rounded-3xl p-6 md:p-8 shadow-2xl relative animate-in fade-in zoom-in-95 duration-300 max-h-[90vh] overflow-y-auto">
            ...
            <form onSubmit={handleUpdateProfile} className="space-y-5">
              ...
              <div>
                ...
                <textarea rows={2} value={editForm.address} onChange={e => setEditForm({...editForm, address: e.target.value})} className={inputStyle} placeholder="Enter your complete address..." />
              </div>
            ...
            <div className="text-center pb-2">
               <button type="button" onClick={handleDeactivateAccount} className="text-red-500 hover:text-red-700 font-bold text-sm transition-colors border border-transparent hover:border-red-200 bg-transparent hover:bg-red-50 px-4 py-2 rounded-lg w-full">
                  {(userData?.tier === 'premium' || userData?.planStatus === 'premium') ? 'Request Account Deactivation' : 'Deactivate & Delete Account'}
               </button>
            </div>
          </div>
        </div>
        ...
        <div className="fixed inset-0 z-[100] flex justify-end bg-black/20 backdrop-blur-sm print:hidden">
          <div className="w-full md:w-[450px] bg-white h-full shadow-2xl flex flex-col animate-in slide-in-from-right duration-300">
            <div className="p-6 border-b border-gray-100 flex justify-between items-center bg-white">
              <div><h2 className="text-xl font-bold text-gray-900">Messages</h2><p className="text-sm font-medium text-gray-500">Inquiries and project requests</p></div>
                ...
                <div key={msg.id} className={`p-4 rounded-2xl cursor-pointer transition-colors border ${msg.unread ? 'bg-white border-[#22c55e]/30 shadow-sm' : 'bg-white border-transparent hover:border-gray-200'}`}>
                  <div className="flex justify-between items-start mb-2">
                    <div className="flex items-center gap-2">
                      ...
                      <div><h4 className="font-bold text-sm text-gray-900">{msg.sender}</h4><span className="text-[10px] font-semibold text-gray-500 uppercase tracking-wider">{msg.role}</span></div>
                    </div>
            ...
            <div className="p-4 border-t border-gray-100 bg-white text-center"><p className="text-xs font-medium text-gray-400">End of messages</p></div>
          </div>
        </div>
      )}
    </div>
